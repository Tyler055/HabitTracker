document.addEventListener("DOMContentLoaded", () => {
  ...
  // Create habit card
  function createHabitElement(name, frequency, id = Date.now()) {
    ...
    div.innerHTML = `
      ...
      <div class="habit-details">
        ...
        <div class="habit-streak">
          <div class="current-streak"><span class="label">Current Streak:</span> <span class="value" data-current-streak="0">0 days</span></div>
          <div class="longest-streak"><span class="label">Longest Streak:</span> <span class="value" data-longest-streak="0">0 days</span></div>
        </div>
  ...
  // Streak logic helper function
  function updateStreaks(habit) {
    const currentSpan = habit.querySelector("[data-current-streak]");
    const longestSpan = habit.querySelector("[data-longest-streak]");
    const current = calculateCurrentStreak(habit);
  ...
  function calculateCurrentStreak(habit) {
    const lastCompletionDate = habit.dataset.lastCompletionDate;
    const today = new Date();
    if (!lastCompletionDate) return 0;

    const difference = Math.floor((today - new Date(lastCompletionDate)) / (1000 * 60 * 60 * 24));
